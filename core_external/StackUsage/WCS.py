...
def read_symbols(readelf_path, file):
    from subprocess import check_output
    ...
    output = check_output([readelf_path, "-s", "-W", file]).decode(stdout_encoding)
    lines = output.splitlines()[3:]
    return [to_symbol(line) for line in lines]


def read_obj(readelf_path, tu, call_graph):
    """
    ...
    """
    filename = tu[0:tu.rindex(".")] + obj_ext
    symbols = read_symbols(readelf_path, filename)
        ...
        if s.type == 'FUNC':
            if s.binding == 'GLOBAL':
                ...
                if s.name in call_graph['globals'] or s.name in call_graph['locals']:
                    print(s)
                    print(call_graph['globals'][s.name])
                    raise Exception('Multiple declarations of {} in {}'.format(s.name, filename))
                ...
                raise Exception('Error Unknown Binding "{}" for symbol: {} in {}'.format(s.binding, s.name, filename))
...
def find_rtl_ext(obj_dirs):
    # Find the rtl_extension
    global rtl_ext
    
    for obj_dir in obj_dirs.split(','):
        for _, _, filenames in os.walk(obj_dir):
            for f in filenames:
...
def find_files(obj_dirs):
    tu = []
    manual = []
    all_files = []
    for obj_dir in obj_dirs.split(','):
        print(obj_dir)
        for root, _, filenames in os.walk(obj_dir):
...
def main(obj_dirs, readelf_executable):

    # Find the appropriate RTL extension
    find_rtl_ext(obj_dirs)

    # Find all input files
    call_graph = {'locals': {}, 'globals': {}, 'weak': {}}
    tu_list, manual_list = find_files(obj_dirs)

    # Read the input files
    for tu in tu_list:
        read_obj(readelf_executable, tu, call_graph)  # This must be first
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Print Stack Usage for given executable')
    parser.add_argument('--objs', dest='obj_dirs', type=str, default='.', help='Comma seperated directories containing object files')
    parser.add_argument('--readelf', dest='readelf', type=str, default='arm-none-eabi-readelf.exe', help='readelf executable')
    args = parser.parse_args()

    main(args.obj_dirs, args.readelf)