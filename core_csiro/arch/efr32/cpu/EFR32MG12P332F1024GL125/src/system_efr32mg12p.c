...
uint32_t SystemCoreClockGet( void )
{
	...
	presc = ( CMU->HFCOREPRESC & _CMU_HFCOREPRESC_PRESC_MASK ) >> _CMU_HFCOREPRESC_PRESC_SHIFT;
	ret /= presc + 1U;
...
uint32_t SystemHFClockGet( void )
{
	...
	return ret / ( 1U + ( ( CMU->HFPRESC & _CMU_HFPRESC_PRESC_MASK ) >> _CMU_HFPRESC_PRESC_SHIFT ) );
}
